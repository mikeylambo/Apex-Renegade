...
export class WorldAtmosphere {
  constructor(engine) {
    ...
    this.sky=new THREE.Mesh(new THREE.SphereGeometry(420,48,28),new THREE.MeshBasicMaterial({map:makeSkyTexture(),side:THREE.BackSide,fog:false})); this.group.add(this.sky);
    this.cloudDome=new THREE.Mesh(new THREE.SphereGeometry(405,48,28),new THREE.MeshBasicMaterial({map:makeCloudTexture(),side:THREE.BackSide,transparent:true,opacity:.92,depthWrite:false,fog:false}));this.cloudDome.rotation.y=.5;this.group.add(this.cloudDome);

    const body=new THREE.Mesh(new THREE.SphereGeometry(27,36,24),new THREE.MeshStandardMaterial({color:0x737d88,roughness:.96,metalness:0,emissive:0x152032,emissiveIntensity:.12}));
    body.position.set(-98,70,-196); this.group.add(body);
    const ringMat=new THREE.MeshBasicMaterial({color:0xa2afc0,transparent:true,opacity:.065,depthWrite:false});
    for(const [r,t,rot] of [[36,.30,[.55,.2,.2]],[43,.13,[.15,.75,.1]],[49,.075,[.8,.2,.7]]]) { const ring=new THREE.Mesh(new THREE.TorusGeometry(r,t,8,96),ringMat.clone()); ring.position.copy(body.position); ring.rotation.set(...rot); this.group.add(ring); }
    ...
    fracture.position.set(-82,60,-164); fracture.scale.set(54,54,1); this.group.add(fracture);
    ...
    for(let i=0;i<4;i++){
      ...
      const ring=new THREE.Mesh(new THREE.TorusGeometry(74+i*9,.09+i*.025,6,128,Math.PI*1.18),mat);ring.position.set(-40,40,-190-i*8);ring.rotation.set(.75,.32,-.35+i*.11);this.group.add(ring);this.pressureRings.push(ring);
    }
    ...
    for(let i=0;i<46;i++) {
      const a=i/46*Math.PI*2, d=105+Math.random()*95, h=16+Math.random()*52, w=4+Math.random()*12;
      const root=new THREE.Group(); root.position.set(Math.cos(a)*d,0,Math.sin(a)*d); root.rotation.y=-a; this.horizon.add(root);
      const tower=new THREE.Mesh(new THREE.BoxGeometry(w,h,5+Math.random()*9),silhouette); tower.position.y=h/2-2; root.add(tower);
      if(i%3===0){const crown=new THREE.Mesh(new THREE.ConeGeometry(w*.62,9+Math.random()*13,4),silhouette);crown.position.y=h+3.5;crown.rotation.y=Math.PI/4;root.add(crown);}
      ...
      if(i%7===0){const antenna=new THREE.Mesh(new THREE.CylinderGeometry(.12,.22,10,6),silhouette);antenna.position.y=h+7;root.add(antenna);}
    }

    const tex=radialTexture('rgba(225,235,247,1)','rgba(110,127,154,.30)');
    const count=1350, geo=new THREE.BufferGeometry(), p=new Float32Array(count*3), speeds=new Float32Array(count);
    for(let i=0;i<count;i++){p[i*3]=(Math.random()-.5)*130;p[i*3+1]=Math.random()*32;p[i*3+2]=(Math.random()-.5)*130;speeds[i]=.035+Math.random()*.10;}
    ...
    for(let i=0;i<28;i++){const s=new THREE.Sprite(new THREE.SpriteMaterial({map:hazeTex,transparent:true,opacity:.13,depthWrite:false,fog:false}));const a=i/28*Math.PI*2,d=42+Math.random()*90;s.position.set(Math.cos(a)*d,5+Math.random()*14,Math.sin(a)*d);s.scale.set(42+Math.random()*72,14+Math.random()*28,1);this.group.add(s);this.haze.push(s);}
  }
  update(dt) {
    ...
    for(let i=0;i<p.count;i++){let y=p.getY(i)+dt*this.speeds[i];let x=p.getX(i)+dt*.008*Math.sin(i);if(y>32)y-=32;if(x>65)x-=130;p.setXYZ(i,x,y,p.getZ(i));} p.needsUpdate=true;
  }
}
