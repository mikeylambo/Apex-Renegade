...
export class PlayerController {
  constructor(engine, input) {
    ...
    const colliderDesc = RAPIER.ColliderDesc.capsule((CFG.standHeight - CFG.radius * 2) / 2, CFG.radius).setTranslation(0, CFG.standHeight / 2, 0);
  ...
  fixedUpdate(dt) {
    ...
    if (this.dashTimer > 0) this.velocity.copy(this.dashDir).multiplyScalar(CFG.dashSpeed);
    else if (this.onWall) this._updateWallRun(input, dt);
    else if (this.grounded) this._groundMove(wishDir, input, dt);
    else { this._airMove(wishDir, dt); this._tryWallRun(input, dt); }

    if (!this.grounded && !this.onWall && this.dashTimer <= 0) {
      const gravityScale = this.onWall ? CFG.wallRunGravity : CFG.gravity;
      this.velocity.y -= gravityScale * dt;
    }

    if (this.grounded && input.isDown('Space') && this.dashTimer <= 0) {
  ...
  _airMove(wishDir, dt) {
    const airAccelMult = GameState.ferocityTier >= 2 ? 1.3 : 1;
  ...
  _updateDash(input, wishDir, dt) {
    ...
    if (dashKey && !input._dashPressed && this.dashCooldownTimer <= 0) {
      ...
      const cooldownMult = GameState.ferocityTier >= 1 ? 0.8 : 1;
  ...
  _updateWallRun(input, dt) {
    ...
    const maxWallRunTime = CFG.wallRunMaxTime * (GameState.ferocityTier >= 2 ? 1.4 : 1);
