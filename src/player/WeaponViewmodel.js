...
function buildPistol(mats, side=1) {
  ...
  root.userData.bolt=bolt; root.userData.boltCore=boltCore;
...
export class WeaponViewmodel {
  constructor(engine,camera,mats) {
    ...
    this.kickL=0; this.kickR=0; this.t=0; this.reloadTimer=0; this.reloadDuration=1;
    ...
    bus.on('reloadStart',({name,duration})=>{if(name==='Corona Blaster'){this.reloadTimer=duration;this.reloadDuration=Math.max(.001,duration);}});
  }
  _ejectCasing(side){
  ...
  update(dt,player) {
    ...
    let reload=0;
    if(this.reloadTimer>0){this.reloadTimer=Math.max(0,this.reloadTimer-dt);const p=1-this.reloadTimer/this.reloadDuration;reload=Math.sin(p*Math.PI);}
    this.leftArm.position.z=-1.00+this.kickL*.13; this.rightArm.position.z=-1.00+this.kickR*.13;
    this.leftArm.position.x=-.38-reload*.12;this.rightArm.position.x=.38+reload*.12;
    this.leftArm.position.y=-.40-reload*.20;this.rightArm.position.y=-.40-reload*.20;
    this.leftArm.rotation.x=-.035-this.kickL*.13+reload*.19; this.rightArm.rotation.x=-.035-this.kickR*.13+reload*.19;
    this.leftArm.rotation.z=-.032-reload*.17;this.rightArm.rotation.z=.032+reload*.17;

    const mech=[[this.left,this.kickL],[this.right,this.kickR]];
    for(const [gun,k] of mech){if(gun.userData.bolt){gun.userData.bolt.position.z=-.16+k*.16;gun.userData.boltCore.position.z=-.16+k*.16;}}
